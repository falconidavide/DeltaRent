...
public class GestioneAccount extends JPanel {
	...
	private JPanel createUserPanel() {
		...
		Boolean isPrivato = Utente.isPrivato();
		if (isPrivato) {
		...
		gbc.gridy++;
		addUserInfo(userPanel, "Totale Veicoli Noleggiati",
				Integer.toString(TotaleNoleggi.getTotaleNoleggi(HomePage.loggedUser)), gbc);
	...
	// Crea il pannello centrale che contiene la modifica dell'email e della
	// password
	...
	private JPanel createPasswordPanel() {
		...
		/*
		 * txtNewPassword.addActionListener(e -> { System.out.println(String.valueOf(
		 ...
		 * )) { btnChangePassword.setEnabled(true); } else {
		 * btnChangePassword.setEnabled(false); } }); txtOldPassword.addActionListener(e
		 * -> { if(Validatori.isValidPassword( String.valueOf(
		 * txtNewPassword.getPassword() ) )) { btnChangePassword.setEnabled(true); }
		 * else { btnChangePassword.setEnabled(false); } });
		 */
		...
		if (newEmail.isEmpty() || currentEmail.isEmpty()) {
			...
			try (PreparedStatement ps = connection.prepareStatement("UPDATE Utente SET email = ? WHERE email = ?")) {
				...
				if (ex.getMessage().contains("UNIQUE") || ex.getMessage().contains("PRIMARY KEY")) {
					...
					JOptionPane.showMessageDialog(this, "Errore nel database." + ex.getMessage(), "Errore",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	...
	DocumentListener passwordListener = new DocumentListener() {
		...
		public void updatePasswordButton() {
			if (Validatori.isValidPassword(String.valueOf(txtNewPassword.getPassword()))
					&& !"".equals(String.valueOf(txtOldPassword.getPassword()))) {
